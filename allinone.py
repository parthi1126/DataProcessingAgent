# test_all_in_one.py
# This file contains ALL code in one place to bypass any caching issues.
...
if GOOGLE_API_KEY:
    try:
        ...
        print("✓ Using real Gemini LLM (gemini-1.5-pro-latest)")
    except Exception as e:
        print(f"Warning: LLM setup failed ({e}). Using mock report.")
        llm = None # Ensure llm is None on failure
else:
    print("WARNING: GOOGLE_API_KEY not found in .env. Using mock report.")
    # llm is already None


# --- 2. ALL FUNCTIONS (Fixed utils.py code) ---
...
def combine_analysis_outputs(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # --- FIX for Test 5: Correctly assign the preview dict ---
...
# --- 3. TEST EXECUTION (Fixed testing_utils.py code) ---

# Your local file path
file_path = r"E:\downloads\archive (17)\surat_uncleaned.csv"  # Use raw string for Windows paths

# Test 1: Ingest Data
print("TEST 1: Ingesting data...")
try:
    result = ingest_data(file_path)
    df = result['data']
    preview = result['preview']
    print(f"✓ Ingestion successful. Shape: {preview['shape']}, Size: {preview.get('size_mb', 'N/A')} MB")
    print("Dtypes:", {k: str(v) for k, v in preview['dtypes'].items()})
    print("Sample Rows (first 2):", preview['sample_rows'][:2])
    
    # Check if 'price_per_sqft' exists before trying to access its missing count
    numeric_cols = [col for col, dtype in preview['dtypes'].items() if str(dtype).startswith('float')]
    print("Numeric Columns:", numeric_cols)
    
    # Check 'square_feet'
    if 'square_feet' in preview['missing']:
        if preview['missing']['square_feet'] >= preview['shape'][0] * 0.9:  # 90%+ null
            print("WARNING: 'square_feet' has high missing values.")
        else:
            print("✓ 'square_feet' seems to be parsing correctly now.")
            print(f"   Missing count: {preview['missing']['square_feet']}")
            print(f"   Sample parsed values: {df['square_feet'].dropna().head(5).to_list()}")
    
    # Check 'price_per_sqft'
    if 'price_per_sqft' in preview['missing']:
        if preview['missing']['price_per_sqft'] >= preview['shape'][0] * 0.9:  # 90%+ null
            print("WARNING: 'price_per_sqft' has high missing values.")
        else:
            print("✓ 'price_per_sqft' seems to be parsing correctly now.")
            print(f"   Missing count: {preview['missing']['price_per_sqft']}")
            print(f"   Sample parsed values: {df['price_per_sqft'].dropna().head(5).to_list()}")
            
except Exception as e:
    print(f"✗ Ingestion failed: {e}")
    # We must stop if ingestion fails.
    exit()

# Enhanced preview update (check if df exists first)
if 'df' in locals():
    preview.update({
        'memory_mb': round(df.memory_usage(deep=True).sum() / (1024**2), 2),
        'file_size_mb': preview.get('size_mb', 0)
    })

    # Test 2: Generate Summary Report
    print("\nTEST 2: Generating summary report...")
    try:
        summary = generate_summary_report(preview)
    ...
    # Test 3: Analyze Data
    print("\nTEST 3: Analyzing data...")
    try:
        analysis = analyze_data(df, file_path=file_path)
        print("Analysis Dict Keys:", list(analysis.keys()))
        print("\nGeneral Info:", analysis['general'])
        print("\nMissing Values:", analysis['missing'])
        print("\nDuplicates:", analysis['duplicates'])
        print("\nOutliers:", analysis['outliers'])
        print("\nDomain Flags:", analysis['domain_flags'])
        print("\nCorrelation:", analysis['correlation'])
        print("\nNumeric Summary:", analysis.get('numeric_summary', 'N/A'))
        print("\nCategorical Summary Keys:", list(analysis.get('categorical_summary', {}).keys())[:3], "...")
        print("\nVisual Insights:", analysis.get('visual_insights', 'N/A'))
    ...
    try:
        # This test now uses the llm loaded from the .env file
        report = generate_quality_report(preview, analysis, llm=llm)
        # Note: We are only printing the first 500 characters so it doesn't flood the terminal.
        # The full report *is* being generated in the 'report' variable.
...
# Test 5: Combine All Outputs
# This test relies on the 'llm' variable inside this script.
print("\nTEST 5: Combining all outputs...")
try:
    combined_output = combine_analysis_outputs(file_path)
    if 'error' in combined_output:
        print(f"✗ Combined output failed during run: {combined_output['error']}")
    else:
        print("Combined Output Keys:", list(combined_output.keys()))
        print("\nIngestion Preview Shape:", combined_output['ingestion']['shape'])
        print("\nSummary:", combined_output['summary'])
        print("\nAnalysis Keys:", list(combined_output['analysis'].keys()))
        print("\nQuality Report (first 200 chars):", combined_output['quality_report'][:200])
        print("\nSample Rows (first 2):", combined_output['sample_rows'][:2])
        print("✓ Combined output successful.")
except Exception as e:
    print(f"✗ Combined output failed: {e}")
    raise

print("\n" + "="*50 + "\n")
print("ALL TESTS COMPLETED! Check console for details.")
print("\n💡 Next: Now that this is fully working, we can build agent.py!")